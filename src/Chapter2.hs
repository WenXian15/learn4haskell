...
firstHalf :: [a] -> [a]
firstHalf l = take num l
  where num = (length l) `div`2
...
False
-}
isThird42 :: (Eq a, Num a, Ord a) => [a] -> Bool
isThird42 (_:_:x:xs)
  | x == 42 =  True
  | otherwise = False 
...
-}
takeEven :: [Int] -> [Int]
takeEven xs = [ x | (n,x) <- zip [0..] xs, even n ]
...
smartReplicate [] = []
smartReplicate (x:xs) = (concat $ map (replicate x) [x]) ++ smartReplicate xs
...
contains one two = [ xs | (n,xs) <- zip test1 two, test2 n]
  where test1 = map (one `elem`) two
        test2 = (\x -> if (x == True) then True else False)
...
-}
rotate :: Int -> [Int] -> [Int]
rotate x y = drop x $ take (x+ length y) $ cycle y 
